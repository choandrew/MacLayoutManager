...
/// Rects here use global Accessibility coordinates: origin at the main display's top-left, y pointing down.
public struct LiveDisplay: Sendable {
    ...
    public init(id: DisplayID, name: String, frame: CGRect, visibleFrame: CGRect) {
        self.id = id
        self.name = name
        self.frame = frame
...
public struct DisplayArrangement: Sendable {
    public let main: LiveDisplay
    public let secondary: [LiveDisplay]

    public var all: [LiveDisplay] { [main] + secondary }
    public var ids: Set<DisplayID> { Set(all.map(\.id)) }
    ...
    public init?(displays: [LiveDisplay]) {
        var ids = displays.map(\.id)
        var occurrences: [DisplayID: Int] = [:]
        for i in displays.indices.sorted(by: { (displays[$0].frame.minX, displays[$0].frame.minY) < (displays[$1].frame.minX, displays[$1].frame.minY) }) {
        ...
        let unique = zip(displays, ids).map { LiveDisplay(id: $1, name: $0.name, frame: $0.frame, visibleFrame: $0.visibleFrame) }
    ...
    func display(containing frame: CGRect) -> LiveDisplay {
        secondary.reduce(main) { best, display in
            display.frame.intersection(frame).area > best.frame.intersection(frame).area ? display : best
        }
    }
}

public struct LiveWindow<Handle> {
    public let handle: Handle
    public let bundleID: String
    public let title: String
    public let frame: CGRect

    public init(handle: Handle, bundleID: String, title: String, frame: CGRect) {
        self.handle = handle
...
public struct WindowMove<Handle> {
    public let handle: Handle
    public let frame: CGRect
}

public func captureScreens<Handle>(of windows: [LiveWindow<Handle>], on displays: DisplayArrangement) -> [ScreenLayout] {
    let windowsByDisplay = Dictionary(grouping: windows) { displays.display(containing: $0.frame).id }
    return displays.all.map { display in
        ScreenLayout(
            display: display.id,
            displayName: display.name,
            windows: windowsByDisplay[display.id, default: []].map {
                WindowPlacement(bundleID: $0.bundleID, title: $0.title, relativeFrame: $0.frame.relative(to: display.visibleFrame))
            }
        )
    }
}
...
public func restorePlan<Handle>(for layout: Layout, windows: [LiveWindow<Handle>], displays: DisplayArrangement) -> [WindowMove<Handle>] {
    ...
    let targets = layout.screens.flatMap { screen in
        ...
        return screen.windows.map { (placement: $0, frame: $0.relativeFrame.absolute(in: visibleFrame)) }
    }
    ...
    for target in targets {
        if let i = unclaimed[target.placement.bundleID]?.firstIndex(where: { $0.title == target.placement.title }),
